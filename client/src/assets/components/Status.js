    ...
    return (
      <div>
        ...
        { ((this.props.state.currentStatus === 5) && (this.props.userType === "Admin")) &&
          <div>
            <input type="button" value="Nouvelle session (avec les voteurs)" onClick= { () => this.newSession(false) } /> 
            <input type="button" value="Nouvelle session (sans voteurs)" onClick= { () => this.newSession(true) } /> 
          </div>
        }
